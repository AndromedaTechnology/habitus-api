...
describe("tag.routes", () => {
  ...
  it("findAll", async () => {
    const response = await request.get(config.api_prefix + "/tags");
  ...
  it("find", async () => {
    const response = await request.get(config.api_prefix + `/tags/${itemId}`);
  ...
  it("update", async () => {
    const response = await request
      .patch(config.api_prefix + `/tags/${itemId}`)
      .set("Authorization", `Bearer ${token}`)
      .send({
  ...
  it("delete", async () => {
    const response = await request
      .delete(config.api_prefix + `/tags/${itemId}`)
      .set("Authorization", `Bearer ${token}`);
